...
function get_the_breadcrumb(){
    ...
    if(is_home()||is_front_page()){
        ...
        // 2019.10.10
        // 原因→シングルページでは最下層のカテゴリを取得する必要があるが、カテゴリの取得はID順に昇順で行われるため、配列数-1で必ず最下層のカテゴリが取得できるとは限らない
        // 応急処置として「親カテゴリが存在する場合は最下層カテゴリ」という判断方法があるがカテゴリが3階層以上になると対応できなくなる

        // 2020.1.11
        // 現在1つの商品に対して親、子、両方のチェックを入れてもらっているが、WPの本来の使い方として
        // その商品の最下層カテゴリだけをチェックすれば良い。
        // そうすればパンクズページでもget_the_category()[0]で常に最下層のカテゴリを取得できるので親カテゴリを再起的に取得すれば良いし、
        // category.phpでは該当カテゴリの子カテゴリもすべて取得している。
        // 
        // 逆にカテゴリを親子階層の全てのカテゴリにつけてしまうと、最下層カテゴリの判定が困難になるので推奨しない。
        // 現在このサイトは親・子両方にチェックを入れてもらってしまっているが、カテゴリ階層はこれ以上増やさない前提なのでとりあえずは下記のコードでも対応できている。
